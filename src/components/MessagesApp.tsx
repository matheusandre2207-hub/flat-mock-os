...
import { generateLocalAIReply } from '../ai/LocalAIEngine';
...
export default function MessagesApp({ 
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    const responseId = setTimeout(async () => {
      ...
      // Try fetching from the server-side Gemini API (skip on static hosts like GitHub Pages)
      ...
      if (!responseText) {
        if (targetChatId === 'self-notes') {
          responseText = "📝 Nota registrada nas anotações do sistema.";
        } else {
          responseText = generateLocalAIReply(targetChatId, userMsgText, latestActiveChat.messages);
        }
      }
